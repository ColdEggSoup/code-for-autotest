...
def test_avidemux_minimize_encode_dialog_to_tray_clicks_tray_button(monkeypatch) -> None:
    ...
    monkeypatch.setattr(software_operations, "bring_window_to_front", lambda *args, **kwargs: None)
    monkeypatch.setattr(software_operations, "find_text_control", lambda *args, **kwargs: tray_button)
    monkeypatch.setattr(software_operations, "click_control", lambda control, post_click_sleep=0.3: control.click_input())
...
def test_handbrake_select_preset_uses_combo_dropdown_when_needed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(operator, "_iter_desktop_wrappers", lambda: [dropdown_item])
    monkeypatch.setattr(software_operations, "bring_window_to_front", lambda *args, **kwargs: None)
    monkeypatch.setattr(software_operations.time, "sleep", lambda _seconds: None)
...
def test_kdenlive_perform_minimizes_immediately_after_render_start(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(software_operations, "bring_window_to_front", lambda *args, **kwargs: None)
    monkeypatch.setattr(operator, "_open_import_dialog", lambda current_window: steps.append(("open_import_dialog", current_window)))
    monkeypatch.setattr(
        software_operations,
        "fill_file_dialog",
        lambda path, **kwargs: steps.append(("fill_file_dialog", path)),
    ...
    monkeypatch.setattr(software_operations.time, "sleep", lambda _seconds: None)
    monkeypatch.setattr(operator, "_wait_for_imported_clip", lambda current_window, path: steps.append(("wait_clip", path)) or clip_item)
    ...
    monkeypatch.setattr(operator, "_open_render_dialog", lambda current_window: steps.append(("open_render_dialog", current_window)) or render_dialog)
    monkeypatch.setattr(operator, "_set_render_output_path", lambda current_dialog, path: steps.append(("set_render_output", path)))
    monkeypatch.setattr(operator, "_show_job_queue_tab", lambda current_dialog: steps.append(("show_job_queue", current_dialog)))
    monkeypatch.setattr(operator, "_start_render", lambda current_dialog: steps.append(("start_render", current_dialog)))
    monkeypatch.setattr(
        operator,
        "_begin_background_wait",
        lambda current_dialog, phase="", start_waiter=None, start_description=None: steps.append(("minimize", phase)),
    )
    monkeypatch.setattr(operator, "_wait_for_render_completion", lambda current_dialog, path: steps.append(("wait_complete", path)))
    monkeypatch.setattr(operator, "close", lambda: steps.append(("close", None)))
