...
HAND_BRAKE_SOURCE_LOAD_TIMEOUT_SECONDS = 20.0
...
HAND_BRAKE_VIDEO_ENCODER_TOP_MAX = 590
HAND_BRAKE_VIDEO_ENCODER_LEFT_MIN = 500
HAND_BRAKE_VIDEO_ENCODER_LEFT_MAX = 760
HAND_BRAKE_TOP_ACTION_BUTTON_TOP_MIN = 220
HAND_BRAKE_TOP_ACTION_BUTTON_TOP_MAX = 300
HAND_BRAKE_START_BUTTON_LEFT_MIN = 640
HAND_BRAKE_START_BUTTON_LEFT_MAX = 860
...
class ShotcutOperator(SoftwareOperator):
    def perform(self, input_video_path: Path, output_video_path: Path) -> None:
        ...
        self._assert_task_queued(toolbar, jobs_dock, output_video_path)
        self._minimize_window_during_wait(window, phase="export")
        logger.info("Waiting for Shotcut export completion.")
        self._wait_for_export_completion(jobs_dock, output_video_path)
        self.close()
    ...
    def _find_child_by_class_name(self, window, class_name: str):
        ...
        for child in window.children():
            ...
            if child_class == class_name:
                return child
        raise AssertionError(f"Shotcut control '{class_name}' was not found.")
    ...
    def _assert_task_queued(self, toolbar, jobs_dock, output_video_path: Path) -> None:
...
class HandBrakeOperator(SoftwareOperator):
    ...
    def perform(self, input_video_path: Path, output_video_path: Path) -> None:
        ...
        bring_window_to_front(window, keep_topmost=False)
        logger.info("Ensuring the HandBrake encoder resolves to x264.")
        self._ensure_x264_encoder(window)
    ...
    def _save_path_edit(self, window):
        ...
        for wrapper in self._iter_wrapper_tree(window):
            if self._wrapper_control_type(wrapper).lower() != "edit":
                continue
            rect = self._wrapper_rect(wrapper)
            if rect.top < HAND_BRAKE_SAVE_PATH_EDIT_TOP_MIN or rect.bottom > HAND_BRAKE_SAVE_PATH_EDIT_TOP_MAX:
                continue
            width = rect.right - rect.left
    ...
    def _source_loaded(self, window, input_video_path: Path) -> bool:
        ...
        if not save_path:
            return False
        return save_path.casefold().endswith(f"{input_video_path.stem.casefold()}.mp4")
    ...
    def _video_encoder_combo(self, window):
        ...
        for wrapper in self._iter_wrapper_tree(window):
            if self._wrapper_control_type(wrapper).lower() != "combobox":
                continue
            rect = self._wrapper_rect(wrapper)
            if rect.top < HAND_BRAKE_VIDEO_ENCODER_TOP_MIN or rect.bottom > HAND_BRAKE_VIDEO_ENCODER_TOP_MAX:
                continue
            if rect.left < HAND_BRAKE_VIDEO_ENCODER_LEFT_MIN or rect.right > HAND_BRAKE_VIDEO_ENCODER_LEFT_MAX:
                continue
            candidates.append(((rect.top, rect.left), wrapper))
    ...
    def _start_encode(self, window, output_video_path: Path) -> None:
        start_button = self._find_button(
            window,
            HAND_BRAKE_START_BUTTON_PATTERNS,
            top_min=HAND_BRAKE_TOP_ACTION_BUTTON_TOP_MIN,
            top_max=HAND_BRAKE_TOP_ACTION_BUTTON_TOP_MAX,
            left_min=HAND_BRAKE_START_BUTTON_LEFT_MIN,
            left_max=HAND_BRAKE_START_BUTTON_LEFT_MAX,
        )
...
class ShutterEncoderOperator(SoftwareOperator):
    ...
    def _wait_until_closed(self, timeout: float = SHUTTER_ENCODER_CLOSE_TIMEOUT_SECONDS) -> bool:
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            try:
                window = self._connect_main_window(timeout=0.5)
    ...
    def perform(self, input_video_path: Path, output_video_path: Path) -> None:
        ...
        self._wait_for_job_to_start(input_video_path, known_outputs)
        self._minimize_window_during_wait(window, phase="transcode")
...
class KdenliveOperator(SoftwareOperator):
    ...
    def _open_render_dialog(self, window):
        ...
        try:
            top_level = render_button.top_level_parent()
            bring_window_to_front(top_level, keep_topmost=False)
            if self._main_window is not None and top_level.handle == self._main_window.handle:
                self._render_dialog = None
            else:
                self._render_dialog = top_level
            logger.info("Resolved Kdenlive rendering container from desktop button search. top_level=%s", top_level.window_text())
        except Exception:
            self._render_dialog = None
            logger.info("Resolved Kdenlive rendering container from desktop button search without a dedicated top-level window.")
    ...
    def _wait_for_render_completion(self, render_dialog, output_video_path: Path) -> None:
        ...
        self._minimize_window_during_wait(render_dialog, phase="render")
